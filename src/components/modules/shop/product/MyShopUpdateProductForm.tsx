...
export default function MyShopUpdateProductForm({ product }: { product: IProduct }) {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    const keyFeatures = data.keyFeatures
      .map((feature: { value: string }) => feature.value)
      .filter(Boolean);
    ...
    try {
      ...
      if (res.success) {
        ...
        router.push("/user/my-shop/products");
      } else {
  ...
  return (
    <div className="border border-border/60 bg-card/30 backdrop-blur-md rounded-3xl flex-grow max-w-2xl p-6 shadow-sm">
      <div className="flex items-center space-x-4 mb-5">
        <Logo />
        <h1 className="text-xl font-black">Update Product Info</h1>
      </div>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      className="h-36 resize-none rounded-xl"
                      {...field}
                      value={field.value || ""}
                    />
                  </FormControl>
          ...
          <div>
            <div className="flex justify-between items-center border-b border-border/40 pb-3 my-5">
              <p className="text-primary font-bold text-lg">Available Colors</p>
              <Button
                variant="outline"
          ...
          <div>
            <div className="flex justify-between items-center border-b border-border/40 pb-3 my-5">
              <p className="text-primary font-bold text-lg">Key Features</p>
              <Button
                onClick={addFeatures}
                variant="outline"
                className="h-8 w-8 rounded-full p-0"
          ...
          <div>
            <div className="flex justify-between items-center border-b border-border/40 pb-3 my-5">
              <p className="text-primary font-bold text-lg">Specification</p>
              <Button
                onClick={addSpec}
                variant="outline"
                className="h-8 w-8 rounded-full p-0"
            ...
            {specFields.map((specField, index) => (
              <div
                key={specField.id}
                className="grid grid-cols-1 gap-4 md:grid-cols-2 my-5 border border-border/40 p-4 rounded-2xl"
              >
                <FormField
                  control={form.control}
          ...
          <Button type="submit" className="mt-6 w-full rounded-xl py-5 font-bold" disabled={isSubmitting}>
